...
export function Player({
  ...
  useEffect(() => {
    ...
    audioPlayer.onTrackEnded = () => {
      console.log(
        '[Player] onTrackEnded fired, currentIndex:',
      ...
      if (wasCrossfading) {
        console.log(
          '[Player] onTrackEnded: crossfade was active, marking as completed',
        )
      ...
      if (repeatModeRef.current === 'one') {
        ...
        console.log(
          '[Player] Calculating next track: currentIndex=' +
            currentTrackIndexRef.current +
            ', nextIndex=' +
            nextIndex +
            ', queueLength=' +
        ...
        if (nextIndex >= queueRef.current.length) {
          if (repeatModeRef.current === 'all') {
            ...
            console.log(
              '[Player] End of queue, repeat all - wrapping to index 0',
            )
          } else {
        ...
        console.log(
          '[Player] Advancing to next track:',
          nextTrack.title || nextTrack.file_path,
        )
  ...
  useEffect(() => {
    if (currentTrackIndex >= 0 && queue[currentTrackIndex]) {
      ...
      console.log(
        `[Player] useEffect triggered: loading track index=${currentTrackIndex}, gen=${gen}, track="${track.title || track.file_path}"`,
      )
      console.log(
        `[Player] useEffect: queue length=${queue.length}, track at index is:`,
        {
          id: track.id,
          title: track.title,
      ...
      if (crossfadeCompletedRef.current) {
        console.log(
          '[Player] useEffect: crossfade completed, updating track reference without reloading',
        )
      ...
      ;(async () => {
        try {
          ...
          setError(null)
          console.log(
            `[Player] useEffect: starting loadTrack for "${track.title || track.file_path}"`,
          )
          await audioPlayer.loadTrack(track.file_path, track.id)
          if (gen !== loadGenRef.current) {
            console.log(
              `[Player] useEffect: gen mismatch after loadTrack (${gen} vs ${loadGenRef.current}), aborting`,
            )
            return
          }
          ...
          if (gen !== loadGenRef.current) {
            console.log(
              `[Player] useEffect: gen mismatch in catch (${gen} vs ${loadGenRef.current}), ignoring error`,
            )
            return
          }
          ...
          if (gen === loadGenRef.current) {
            console.log(
              `[Player] useEffect: cleaning up, setting isLoading=false`,
            )
            setIsLoading(false)
          }
        }
      ...
      console.warn(
        `[Player] useEffect: invalid state - currentTrackIndex=${currentTrackIndex} but no track in queue`,
      )
    }
  }, [
    currentTrackIndex,
  ...
  useEffect(() => {
    ...
    if (timeUntilEnd > 0 && timeUntilEnd <= crossfadeDurationMs + 500) {
      ...
      if (nextTrack) {
        console.log(
          `[Player] Attempting crossfade: ${currentTrack.title} → ${nextTrack.title}`,
        )
      ...
      <div className="sc-player__bar">
        ...
        <div className="sc-player__controls">
          ...
          >
            ...
            {repeatMode === 'one' && (
              <span className="sc-player__repeat-badge">1</span>
            )}
          </button>
        </div>
        ...
        <div className="sc-player__right">
          ...
          <div
            ...
            title={
              currentTrack ? 'Click to scroll to track in library' : undefined
            }
          >
            {currentTrack ? (
              <>
              ...
              <>
                <span className="sc-player__label sc-player__label--empty">
                  No track loaded
                </span>
          ...
          <button
            ...
            onClick={() =>
              import('../lib/miniPlayer')
                ...
                .catch((err) =>
                  console.error('[Player] Mini player open failed:', err),
                )
            }
          ...
          <div className="sc-player__playlist-wrapper" ref={playlistMenuRef}>
            ...
            {showPlaylistMenu && (
              <div className="sc-player__playlist-menu">
                <div className="sc-player__playlist-header">
                  Add to playlist
                </div>
                {manualPlaylists.length > 0 ? (
                  manualPlaylists.map((p) => (
                    ...
                    >
                      ...
                      />
                      <span>{p.name}</span>
                      <span className="sc-player__playlist-count">
                        {p.track_count}
                      </span>
                    </button>
                  ))
                ) : (
                  <div className="sc-player__playlist-empty">
