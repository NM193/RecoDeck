...
const WAVEFORM_STYLES = [
  {
    id: 'traktor_rgb',
    name: 'Traktor RGB',
    description: 'Colored frequency bands (bass/mid/treble)',
  },
  {
    id: 'mono_peaks',
    name: 'Mono Peaks',
    description: 'Classic single-color waveform',
  },
...
export function Settings({
  ...
  // AI settings
  const { isApiKeyConfigured, checkApiKeyStatus, setApiKey, deleteApiKey } =
    useAIStore()
  ...
  async function loadSettings() {
    try {
      ...
      setCustomColors(
        loadedCustomColors
          ? { ...DEFAULT_CUSTOM_COLORS, ...loadedCustomColors }
          : DEFAULT_CUSTOM_COLORS,
      )
  ...
  async function handleAddFolder() {
    try {
      ...
      const updatedFolders = await tauriApi.addLibraryFolder(
  ...
  async function handleSaveApiKey() {
    ...
    try {
      ...
      setApiKeyInput('')
      onNotification?.(
        'API key saved successfully! You can now use the AI assistant.',
        'success',
      )
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : String(err)
      ...
      if (update) {
        ...
        await update.downloadAndInstall((event) => {
          if (event.event === 'Started') {
            ...
            setUpdateProgress((p) =>
              p ? { ...p, status: 'installing', progress: 98 } : p,
            )
          }
        })

        setUpdateProgress((p) =>
          p ? { ...p, status: 'installing', progress: 100 } : p,
        )
        ...
        if (restartNow) {
          ...
          onNotification?.(
            'Update will be applied when you restart the app.',
            'info',
          )
        }
      } else {
          ...
          <div className="settings-custom-colors">
            ...
            <div className="custom-colors-grid">
              {(
                Object.keys(
                  DEFAULT_CUSTOM_COLORS,
                ) as (keyof typeof DEFAULT_CUSTOM_COLORS)[]
              ).map((key) => (
        ...
        <div className="key-notation-list">
          {KEY_NOTATIONS.map((notation) => (
            ...
            >
              ...
              {keyNotation === notation.id && (
                <Icon name="Check" size={16} className="notation-check" />
              )}
            </button>
          ))}
        </div>
        ...
        >
          <div>
            ...
            <p
              ...
              style={{ marginTop: '0.5rem', fontSize: '0.75rem', opacity: 0.7 }}
            >
              Finds and removes duplicate tracks based on file content and
              filename. Keeps the earliest imported version of each track.
            </p>
          </div>
  ...
  const renderAIContent = () => (
    <div className="settings-tab-content">
      ...
      <section className="settings-section">
        <p className="settings-description">
          Configure your Claude API key to enable AI-powered playlist
          generation.
        </p>
        ...
        <div className="settings-input-group" style={{ marginTop: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
            <input
              ...
              placeholder={
                isApiKeyConfigured ? '••••••••••••••••' : 'sk-ant-api03-...'
              }
              value={apiKeyInput}
  ...
  const renderCompanionContent = () => (
    <div className="settings-tab-content">
      ...
      <section className="settings-section">
        <p className="settings-description">
          Stream music from this computer to your phone over WiFi. Open the URL
        ...
        <div style={{ marginTop: '1rem' }}>
          ...
          <button
            onClick={
              companionRunning ? handleStopCompanion : handleStartCompanion
            }
        ...
        {companionRunning && companionUrl && (
          ...
          >
            ...
            >
              ...
              >
                {companionActiveStreams} active stream
                {companionActiveStreams !== 1 ? 's' : ''}
              </span>
            </div>
            ...
            <p
              className="settings-hint"
              style={{ marginTop: '1rem', fontSize: '0.7rem', opacity: 0.6 }}
            >
              Your token and port are saved — your phone stays paired across
              restarts. Regenerating the token will disconnect all connected
              devices.
            </p>
          </div>
        )}
        ...
        >
          Enabled automatically after first manual start. Your phone will
        ...
        <p
          ...
          style={{ marginTop: '1rem', fontSize: '0.75rem', opacity: 0.7 }}
        >
          The server runs on your local network only. Your phone must be
          connected to the same WiFi network. Supported formats: MP3, AAC/M4A,
          WAV. FLAC and OGG may not work on all mobile browsers.
        </p>
      </section>
    </div>
  )
