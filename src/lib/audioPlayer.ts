...
export class AudioPlayer {
  ...
  private setupEventListeners() {
    ...
    audio.addEventListener('ended', () => {
      ...
      console.log(
        '[AudioPlayer] ended event fired, position:',
        this._position,
        'duration:',
        this._duration,
      )
      ...
      if (this.isCrossfading) {
        console.warn(
          '[AudioPlayer] Outgoing track ended during crossfade fade-in, completing swap immediately',
        )
      ...
      if (this.crossfadeFadeComplete) {
        console.log(
          '[AudioPlayer] Outgoing track ended naturally after crossfade, performing swap',
        )
        this.completeCrossfade()
        return
      }
      ...
      const metaGap =
        this._metadataDurationMs > 0
          ...
          : 0
      const browserGap =
        this._duration > this._position ? this._duration - this._position : 0
      ...
      if (this._currentTrackId != null && !this._nativeRecoveryAttempted) {
        ...
        console.log(
          `[AudioPlayer] Attempting native decoder recovery` +
            (maxGap > 1000
              ? ` (~${Math.round(maxGap / 1000)}s may remain).`
              : '.'),
        )
      ...
      source.addEventListener('ended', () => {
        ...
        if (this.nativeSources.size === 0 && this.audioCtx) {
          ...
          if (decoderDone || allScheduledPlayed) {
            console.log(
              '[AudioPlayer] Last source ended — advancing (decoderDone=' +
                decoderDone +
                ')',
            )
  ...
  private nativeEmitEndedOnce() {
    ...
    console.log(
      `[AudioPlayer] Native decoder finished: position=${Math.round(position)}ms, duration=${Math.round(duration)}ms`,
    )

    if (this.audioCtx) {
      const remainingAudio =
        (this.nativeNextStartTime - this.audioCtx.currentTime) * 1000
      console.log(
        `[AudioPlayer] WebAudio buffers remaining: ~${Math.round(remainingAudio)}ms`,
      )
    }
  ...
  private nativeFinishPlayback() {
    ...
    console.log(
      '[AudioPlayer] nativeFinishPlayback: all audio done, advancing to next track',
    )
    this.stopNativePositionTimer()
    this._isPlaying = false
    this.onPlayStateChange?.(false)
    console.log(
      '[AudioPlayer] Calling onTrackEnded, callback defined:',
      !!this.onTrackEnded,
    )
    this.onTrackEnded?.()
  }
  ...
  private startNativePositionTimer() {
    ...
    this.nativePositionTimer = window.setInterval(() => {
      ...
      const pos =
        this.nativeStartPositionMs +
        (this.audioCtx.currentTime - this.nativeStartAudioTime) * 1000
      ...
      if (
        this.nativeSources.size === 0 &&
        this.nativeNextStartTime > this.nativeStartAudioTime + 0.1 &&
        this.audioCtx.currentTime > this.nativeNextStartTime + 0.15
      ) {
        console.log(
          '[AudioPlayer] Timer detect: all audio played, no sources remain',
        )
        if (!this.nativeEndedEmitted) this.nativeEndedEmitted = true
        this.nativeFinishPlayback()
        return
  ...
  private filePathToStreamUrl(filePath: string): string {
    ...
    if (
      trimmed.startsWith('stream://localhost/') ||
      trimmed.startsWith('stream://localhost/?p=')
    ) {
      return trimmed
    }
    if (
      trimmed.startsWith('http://stream.localhost/') ||
      trimmed.startsWith('http://stream.localhost/?p=')
    ) {
      return trimmed
    }
  ...
  async loadTrack(filePath: string, trackId?: number): Promise<void> {
    ...
    try {
      ...
      console.log(
        '[AudioPlayer] loadTrack: cleaning up previous playback, mode:',
        this.mode,
      )
      ...
      if (this.mode === 'native') {
        ...
        try {
          ...
          console.warn(
            '[AudioPlayer] loadTrack: error stopping native playback:',
            err,
          )
        }
        if (gen !== this._loadGeneration) {
          console.log(
            '[AudioPlayer] loadTrack: superseded during native cleanup, aborting',
          )
          return
        }
      }
      ...
      console.log(
        '[AudioPlayer] loadTrack: reset state - position=0, duration=0, isPlaying=false',
      )
      ...
      const webkitUnsupportedFormats = [
        'ogg',
        'opus',
        'wma',
        'ape',
      ...
      if (preferNative && typeof trackId === 'number') {
        console.log(
          `[AudioPlayer] Format .${ext} not supported by WebKit, using native decoder`,
        )
        await this.switchToNative(trackId)
        if (gen !== this._loadGeneration) {
          console.log(
            '[AudioPlayer] loadTrack: superseded after switchToNative, aborting',
          )
          return
        }
      ...
      try {
        ...
        if (gen !== this._loadGeneration) {
          console.log(
            '[AudioPlayer] loadTrack: superseded after loadUrl, aborting',
          )
          return
        }
      } catch (e) {
        if (gen !== this._loadGeneration) {
          console.log(
            '[AudioPlayer] loadTrack: superseded in catch block, aborting',
          )
          return
        }
        // Check if this is a codec/format error (code 4 = MEDIA_ERR_SRC_NOT_SUPPORTED)
        const errCode = (e as { code?: number })?.code
        const isFormatError = errCode === 4 || errCode === 3

        if (isFormatError && typeof trackId === 'number') {
          // Format not supported by WebKit - switch to native decoder immediately
          console.log(
            '[AudioPlayer] WebKit cannot decode this format, switching to native decoder',
          )
          await this.switchToNative(trackId)
          if (gen !== this._loadGeneration) {
            console.log(
              '[AudioPlayer] loadTrack: superseded after switchToNative fallback, aborting',
            )
            return
          }
        ...
        if (fallbackUrl !== url && !isFormatError) {
          ...
          try {
            ...
            if (gen !== this._loadGeneration) {
              console.log(
                '[AudioPlayer] loadTrack: superseded after fallback loadUrl, aborting',
              )
              return
            }
            console.log(
              '[AudioPlayer] loadTrack: fallback URL loaded successfully',
            )
            return
          } catch (e2) {
            if (gen !== this._loadGeneration) {
              console.log(
                '[AudioPlayer] loadTrack: superseded in fallback catch, aborting',
              )
              return
            }
            // If fallback also has format error and we have trackId, try native
            const errCode2 = (e2 as { code?: number })?.code
            const isFormatError2 = errCode2 === 4 || errCode2 === 3
            if (isFormatError2 && typeof trackId === 'number') {
              console.log(
                '[AudioPlayer] Fallback URL also failed with format error, switching to native decoder',
              )
              await this.switchToNative(trackId)
              if (gen !== this._loadGeneration) {
                console.log(
                  '[AudioPlayer] loadTrack: superseded after fallback native switch, aborting',
                )
                return
              }
      ...
      if (gen === this._loadGeneration) {
        ...
        console.log(
          '[AudioPlayer] loadTrack: NOT clearing loading flag (superseded)',
        )
      }
    }
  ...
  private async switchToNative(trackId: number): Promise<void> {
    ...
    const { tauriApi } = await import('./tauri-api')
    console.log(
      '[AudioPlayer] switchToNative: ensuring native audio context...',
    )
    ...
    console.log(
      '[AudioPlayer] switchToNative: calling playbackLoadTrack, trackId:',
      trackId,
    )
    const status = await tauriApi.playbackLoadTrack(trackId)
    console.log(
      '[AudioPlayer] switchToNative: playbackLoadTrack complete, duration:',
      status.duration_ms,
      'ms',
    )
    ...
    this.onPositionUpdate?.(this._position)
    console.log(
      '[AudioPlayer] switchToNative: complete, position:',
      this._position,
      'duration:',
      this._duration,
    )
  ...
  async play(): Promise<void> {
    ...
    try {
      ...
      console.log(
        '[AudioPlayer] play() successful, isPlaying will be set by play event',
      )
    } catch (err) {
  ...
  async startCrossfadeToNext(
    ...
    if (this.mode !== 'html') {
      console.warn(
        '[AudioPlayer] Crossfade only supported in HTML mode, skipping',
      )
    ...
    try {
      ...
      if (
        ...
        console.log(
          `[AudioPlayer] Beatmatch: ${this.outgoingBpm} BPM → ${this.incomingBpm} BPM, playbackRate=${playbackRate.toFixed(3)}`,
        )
      } else {
        console.log(
          '[AudioPlayer] Crossfade without beatmatch (missing BPM data)',
        )
      }
  ...
  private updateCrossfadeVolumes() {
    ...
    if (progress >= 1.0) {
      ...
      if (this.audio.ended) {
        console.log(
          '[AudioPlayer] Outgoing track already ended, completing crossfade swap',
        )
        this.completeCrossfade()
      } else {
        console.log(
          '[AudioPlayer] Crossfade fade-in complete, waiting for outgoing track to end naturally',
        )
      }
    } else {
      // Continue animation
      this.crossfadeRafId = requestAnimationFrame(() =>
        this.updateCrossfadeVolumes(),
      )
    }
  }
