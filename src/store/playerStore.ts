...
export const usePlayerStore = create<PlayerState>((set, get) => ({
  ...
  setQueue: (tracks, startIndex = 0) => {
    console.log(
      `[PlayerStore] setQueue called: ${tracks.length} tracks, startIndex=${startIndex}`,
    )
    if (startIndex >= 0 && startIndex < tracks.length) {
      console.log(
        `[PlayerStore] setQueue: track at index ${startIndex} is "${tracks[startIndex].title || tracks[startIndex].file_path}"`,
      )
    } else {
      console.error(
        `[PlayerStore] setQueue: INVALID INDEX ${startIndex} for queue length ${tracks.length}`,
      )
    }
    set({
      queue: [...tracks],
  ...
  setShuffle: (enabled) => {
    ...
    if (enabled) {
      ...
      if (currentTrack) {
        const newCurrentIndex = shuffled.findIndex(
          (t) => t.id === currentTrack.id,
        )
        if (
          newCurrentIndex !== -1 &&
          newCurrentIndex !== state.currentTrackIndex
        ) {
          ;[shuffled[state.currentTrackIndex], shuffled[newCurrentIndex]] = [
            shuffled[newCurrentIndex],
            shuffled[state.currentTrackIndex],
          ]
        }
      }
